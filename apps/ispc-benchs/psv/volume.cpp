...
STATIC_INLINE void generateRay(const float raster2camera[4][4],
                               const float camera2world[4][4], float x, float y,
                               Ray& ray) {
...
STATIC_INLINE bool Inside(float3 p, float3 pMin, float3 pMax) {
    return (p.x >= pMin.x && p.x <= pMax.x && p.y >= pMin.y && p.y <= pMax.y &&
            p.z >= pMin.z && p.z <= pMax.z);
}

STATIC_INLINE bool IntersectP(const Ray& ray, float3 pMin, float3 pMax,
                              float* hit0, float* hit1) {
...
STATIC_INLINE float Lerp(float t, float a, float b) {
    return (1.f - t) * a + t * b;
}

STATIC_INLINE int Clamp(int v, int low, int high) {
    return std::min(std::max(v, low), high);
}

STATIC_INLINE float D(int x, int y, int z, int nVoxels[3], float density[]) {
...
STATIC_INLINE float3 Offset(float3 p, float3 pMin, float3 pMax) {
    return float3((p.x - pMin.x) / (pMax.x - pMin.x),
                  (p.y - pMin.y) / (pMax.y - pMin.y),
                  (p.z - pMin.z) / (pMax.z - pMin.z));
}

STATIC_INLINE float Density(float3 Pobj, float3 pMin, float3 pMax,
                            float density[], int nVoxels[3]) {
    if (!Inside(Pobj, pMin, pMax)) return 0;
...
STATIC_INLINE float transmittance(float3 p0, float3 p1, float3 pMin,
                                  float3 pMax, float sigma_t, float density[],
                                  int nVoxels[3]) {
    float rayT0, rayT1;
    Ray ray;
    ray.origin = p1;
    ...
    if (!IntersectP(ray, pMin, pMax, &rayT0, &rayT1)) return 1.;
...
STATIC_INLINE float distanceSquared(float3 a, float3 b) {
    float3 d = a - b;
    return d.x * d.x + d.y * d.y + d.z * d.z;
}

STATIC_INLINE float raymarch(float density[], int nVoxels[3], const Ray& ray) {
    float rayT0, rayT1;
    float3 pMin(.3f, -.2f, .3f), pMax(1.8f, 2.3f, 1.8f);
    float3 lightPos(-1.f, 4.f, 1.5f);

    if (!IntersectP(ray, pMin, pMax, &rayT0, &rayT1)) return 0.;
...
void volume_psv(float density[], int nVoxels[3],
...
#psim num_spmd_threads(width) gang_size(16)
        {
            ...
            uint64_t my_offset = offset + x;
            Ray ray;
            generateRay(raster2camera, camera2world, (float)x, (float)y, ray);
            image[my_offset] = raymarch(density, nVoxels, ray);
        }
        offset += width;
    }
}