...
STATIC_INLINE float Clamp(float v, float low, float high) {
    return v < low ? low : ((v > high) ? high : v);
}

STATIC_INLINE float SmoothStep(float low, float high, float value) {
    float v = Clamp((value - low) / (high - low), 0.f, 1.f);
    return v * v * (-2.f * v + 3.f);
}

STATIC_INLINE int Floor2Int(float val) { return (int)floorf(val); }

STATIC_INLINE float Grad(int x, int y, int z, float dx, float dy, float dz) {
...
STATIC_INLINE float NoiseWeight(float t) {
    float t3 = t * t * t;
    float t4 = t3 * t;
    return 6.f * t4 * t - 15.f * t4 + 10.f * t3;
}

STATIC_INLINE float Lerp(float t, float low, float high) {
    return (1.f - t) * low + t * high;
}

STATIC_INLINE float Noise(float x, float y, float z) {
...
STATIC_INLINE float Turbulence(float x, float y, float z, int octaves) {
    float omega = 0.6;
...
void noise_psv(float x0, float y0, float x1, float y1, int width, int height,
               float output[]) {
    float dx = (x1 - x0) / width;
    float dy = (y1 - y0) / height;

    for (int j = 0; j < height; j++) {
#psim num_spmd_threads(width) gang_size(32)
        {
